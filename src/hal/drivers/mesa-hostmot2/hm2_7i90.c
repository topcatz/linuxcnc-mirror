...
//
//    Copyright (C) 2013 Sebastian Kuzminsky and Kim Kirwan
//
...
static int num_ioaddr_his = HM2_7I90_MAX_BOARDS;
module_param_array(ioaddr_hi, int, &num_ioaddr_his, S_IRUGO);
MODULE_PARM_DESC(ioaddr_hi, "secondary address of the parallel port(s) (see hm2_7i90(9) manpage)");

static int epp_wide[HM2_7I90_MAX_BOARDS] = { [0 ... (HM2_7I90_MAX_BOARDS-1)] = 1 };
static int num_epp_wides = HM2_7I90_MAX_BOARDS;
module_param_array(epp_wide, int, &num_epp_wides, S_IRUGO);
MODULE_PARM_DESC(epp_wide, "set to 0 to disable wide EPP mode (see (hm2_7i90(9) manpage)");

int debug_epp = 0;
RTAPI_MP_INT(debug_epp, "Developer/debug use only!  Enable debug logging of most EPP\ntransfers.");

static char *config[HM2_7I90_MAX_BOARDS];
static int num_config_strings = HM2_7I90_MAX_BOARDS;
...
int hm2_7i90_read(hm2_lowlevel_io_t *this, u32 addr, void *buffer, int size) {
    ...
    epp_addr16(addr | HM2_7I90_ADDR_AUTOINCREMENT, &board->epp_port);
...
// FIXME: i think te 7i90 can't be programmed this way
